...
$sql = "
  ...
  WHERE cc.id=? AND cc.user_id=? AND m.user_id=cc.user_id
    AND COALESCE(cc.collection_status, 'pending') <> 'collected'
    AND COALESCE(cc.payment_status, 'unpaid') <> 'paid'
    AND COALESCE(cc.delivery_assignment_status, 'unassigned') <> 'collected'
";
...
if ($stmt->affected_rows === 0) {
  ...
  echo json_encode(["success" => false, "error" => "Customer not found, already collected, or not allowed"]);
  exit;
}
