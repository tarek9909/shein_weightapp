...
try {
  ...
  $stmt = $conn->prepare("
    ...
    SET delivery_number = ?, status = ?, delivery_charge_usd = ?
    WHERE id = ?
  ");
  $status="withdelivery";
  $stmt->bind_param("ssdi", $delivery_number, $status, $delivery_charge_usd, $id);
  ...
  ob_clean();
  error_log('[shein-php] update customer delivery failure: ' . (string)$e);
  echo json_encode(["success" => false, "error" => backend_public_exception_message($e)]);
  exit;
}
