...
router.post(paths("addPayment"), asyncHandler(async (req, res) => {
  ...
  if (paymentType === "customers") {
    ...
    for (const item of items) { if (!item || typeof item !== "object") continue; const customerId = int(item.customer_id); const amount = number(item.amount); const itemCharge = number(item.delivery_charge); const name = trim(item.customer_name); if (!finite(item.amount) || amount < 0 || !finite(item.delivery_charge) || itemCharge < 0 || itemCharge > amount || (customerId <= 0 && !name)) continue; prepared.push({ customer_id: customerId > 0 ? customerId : null, customer_name: name || null, amount, delivery_charge: itemCharge }); original += amount; charge += itemCharge; if (customerId > 0) selected.push(customerId); }
    if (!prepared.length) return res.status(400).json({ success: false, error: "No valid customer items" });
    const db = await pool.getConnection(); try { await db.beginTransaction(); if (selected.length) { const eligible = await rows(db, `SELECT cc.id FROM cart_customers cc JOIN order_carts oc ON oc.id=cc.cart_id AND oc.user_id=cc.user_id JOIN orders o ON o.id=oc.order_id AND o.user_id=oc.user_id WHERE cc.user_id=? AND o.month_id=? AND cc.id IN (${placeholders(selected)}) AND COALESCE(cc.status,'') NOT IN ('paid','done') AND COALESCE(cc.delivery_status,'') NOT IN ('paid','done')`, [uid(req), monthId, ...selected]); const eligibleSet = new Set(eligible.map((r) => Number(r.id))); if (selected.some((id) => !eligibleSet.has(id))) throw new HttpError(400, "One or more selected customers are already paid or not eligible", { success: false, error: "One or more selected customers are already paid or not eligible" }); }
      const net = original - charge; const ids = uniquePositiveInts(selected); const idsJson = JSON.stringify(ids); const note = trim(data.note) || "Customer payment"; const payment = await execute(db, `INSERT INTO payments (user_id,month_id,payment_amount,payment_type,original_amount,delivery_charge,customer_count,customer_ids_json,note) VALUES (?, ?, ?, 'customers', ?, ?, ?, ?, ?)`, [uid(req), monthId, net, original, charge, prepared.length, idsJson, note]); const paymentId = Number(payment.insertId); const chargeColumn = await hasColumn(db, "payment_customer_items", "delivery_charge"); for (const item of prepared) { const sql = chargeColumn ? "INSERT INTO payment_customer_items (payment_id,customer_id,customer_name_snapshot,amount,delivery_charge,user_id) VALUES (?, ?, ?, ?, ?, ?)" : "INSERT INTO payment_customer_items (payment_id,customer_id,customer_name_snapshot,amount,user_id) VALUES (?, ?, ?, ?, ?)"; await execute(db, sql, chargeColumn ? [paymentId,item.customer_id,item.customer_name,item.amount,item.delivery_charge,uid(req)] : [paymentId,item.customer_id,item.customer_name,item.amount,uid(req)]); } if (selected.length) await execute(db, `UPDATE cart_customers SET status='paid',delivery_status='paid' WHERE user_id=? AND id IN (${placeholders(ids)})`, [uid(req), ...ids]); await db.commit(); res.json({ success:true,id:paymentId,payment_type:"customers",payment_amount:net,original_amount:original,delivery_charge:charge,customer_count:prepared.length,customer_ids_json:idsJson,note }); } catch(e) { try { await db.rollback(); } catch (_) {} throw e; } finally { db.release(); }
    return;
  }
